...
namespace librealsense
{
    ...
    class rs438_device : public d400_active,
        public d400_color,
        public d400_motion,
        public ds_advanced_mode_base,
        public firmware_logger_device
    {
    public:
        rs438_device(std::shared_ptr< const d400_info > const& dev_info, bool register_device_notifications)
            : device(dev_info, register_device_notifications)
            , backend_device(dev_info, register_device_notifications)
            , d400_device(dev_info)
            , d400_active(dev_info)
        ...
        std::vector<tagged_profile> get_profiles_tags() const override
        {
            ...
            int depth_width = usb3mode ? 848 : 640;
            int depth_height = usb3mode ? 480 : 480;
            int color_width = usb3mode ? 4160 : 640;
            int color_height = usb3mode ? 3120 : 480;
            int fps = usb3mode ? 15 : 15;
    ...
    std::shared_ptr< device_interface > d400_info::create_device()
    {
        ...
        case RS438_PID:
            return std::make_shared< rs438_device >( dev_info, register_device_notifications );
        default:
    ...
    std::shared_ptr<matcher> rs438_device::create_matcher(const frame_holder& frame) const
    {
        std::vector<stream_interface*> streams = { _depth_stream.get() , _left_ir_stream.get() , _right_ir_stream.get(), _color_stream.get() };
        std::vector<stream_interface*> mm_streams = { _ds_motion_common->get_accel_stream().get(),
                                                      _ds_motion_common->get_gyro_stream().get() };
